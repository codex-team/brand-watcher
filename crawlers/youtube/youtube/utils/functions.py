    ...
    def get_comments(youtube, self):
        try:
            ...
            if "nextPageToken" in video_response:
                return Functions.get_comments(youtube, self.video_id, self.comments, video_response['nextPageToken'])
            else:
                return self.comments
        except Exception:
            return 'Comments Disabled'

    def search_video(keyword, youtube):
        videos = []
        ...
        for item in res['items']:
            video = {
                ...
                'comments': Functions.get_comments(youtube, item['id']['videoId'])
            }
            videos.append(video)
        return videos